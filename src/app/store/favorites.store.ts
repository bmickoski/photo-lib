...
export class FavoritesStore {
  ...
  // Set gives O(1) lookup in isFavorite() — avoids Array.includes() on every render cycle
  readonly ids = computed(() => new Set(this.#favorites().map((p) => p.id)));

  constructor() {
    // effect() re-runs whenever #favorites changes, keeping localStorage in sync automatically
    effect(() => {
